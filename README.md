...
The emulation runs in a separate thread; the main loop outputs entire VRAM in a window and synchronizes using a simple atomic lock. GPU part can draw only colored triangles and quads, no textures support yet. No CD-ROM, no audio, no controller input. Don't expect it to run any games at the current stage.
